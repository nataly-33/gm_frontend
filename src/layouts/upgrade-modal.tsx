...
function CardForm({
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      if (stripeErr) {
        setError(stripeErr.message ?? 'Error al agregar tarjeta')
      } else {
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <button
        onClick={onCancel}
        className="text-xs py-2 text-muted hover:text-white transition-colors"
...
export default function UpgradeModal({ show, onClose, onSuccess }: Props) {
  ...
  async function handlePay(pm: PaymentMethod) {
    ...
    try {
      await clientService.makePayment(
        pm.stripe_payment_method_id,
        parseFloat(selectedPlan.price_usd),
        selectedPlan.slug
      )
  ...
  return (
    ...
    >
      <div
        className="rounded-2xl p-5 sm:p-6 w-full max-w-3xl max-h-[92vh] overflow-y-auto"
      ...
      >
        ...
        {step === 'plans' && (
          <>
            ...
            <p className="text-center text-xs mb-8 text-muted">
                ...
                {plans.map((plan) => {
                  ...
                  return (
                    <div
                      ...
                      style={{
                        background: isPaid
                          ? 'linear-gradient(135deg,#2d1b69 0%,#1a1040 100%)'
                          : '#121212',
                        border: isPaid ? '1px solid #A855F7' : '1px solid #2a2a2a',
                      }}
                    >
                      {isPaid && (
                        <span
                      ...
                      <div className="flex flex-col gap-1.5 flex-1 text-xs text-muted">
                        <p>
                          <span className="text-primary">✓</span> {plan.credits_per_month}{' '}
                          créditos/mes
                        </p>
                      ...
                      <button
                        ...
                        style={{
                          background: isPaid ? '#A855F7' : '#2a2a2a',
            ...
            <button
              onClick={handleClose}
              className="mt-6 w-full text-xs py-2 rounded-lg text-muted"
              style={{ background: 'transparent' }}
            >
              Cerrar
            </button>
        ...
        {step === 'payment-methods' && (
          <>
            <button
              ...
              className="text-muted text-xs mb-4 flex items-center gap-1 hover:text-white transition-colors"
            >
              ← Volver
            </button>
            ...
            <p className="text-xs mb-6 text-muted">
              {selectedPlan?.name} —{' '}
              <span className="text-white font-bold">${selectedPlan?.price_usd}/mes</span>
            </p>
            {loadingMethods ? (
        ...
        {step === 'new-card' && (
          <>
            <button
              onClick={() => setStep('payment-methods')}
              className="text-muted text-xs mb-4 flex items-center gap-1 hover:text-white transition-colors"
            >
              ← Volver
            </button>
