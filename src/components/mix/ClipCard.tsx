  ...
  return (
    <label style={{ display: 'flex', flexDirection: 'column', gap: 4, fontSize: 12, color: 'var(--text-subdued)' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
        ...
        />
        <span style={{ fontSize: 11, minWidth: 40, textAlign: 'right', color: 'var(--text-subdued)' }}>{value}ms</span>
      </div>
    </label>
  )
}
  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <span style={{
            width: 24, height: 24, borderRadius: '50%', background: 'var(--bg-card-hover)',
            display: 'flex', alignItems: 'center', justifyContent: 'center',
            fontSize: 11, fontWeight: 700, color: isSelected ? 'var(--color-primary)' : 'var(--text-subdued)',
            flexShrink: 0,
          }}>
            {clip.position + 1}
          </span>
          <span style={{ fontSize: 14, fontWeight: 600, color: 'var(--text-base)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: 160 }}>
            {title}
          </span>
        </div>
        <button
          onClick={e => { e.stopPropagation(); handleDelete() }}
          style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-subdued)', padding: 4, borderRadius: 4 }}
          title="Eliminar clip"
        >
      ...
      {/* Duration bar */}
      <div style={{ height: 6, borderRadius: 3, background: 'var(--bg-card-hover)', marginBottom: 8, overflow: 'hidden' }}>
        <div style={{ height: '100%', borderRadius: 3, background: isSelected ? 'var(--color-primary)' : '#555', width: '100%' }} />
      </div>

      <p style={{ fontSize: 12, color: 'var(--text-subdued)', margin: 0 }}>
        {(clip.start_time_ms / 1000).toFixed(1)}s – {(clip.end_time_ms / 1000).toFixed(1)}s ({durationSec}s)
      </p>
      ...
      {isSelected && (
        <div
          ...
          style={{ marginTop: 14, display: 'flex', flexDirection: 'column', gap: 10, borderTop: '1px solid var(--border)', paddingTop: 12 }}
        >
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
            <label style={{ display: 'flex', flexDirection: 'column', gap: 4, fontSize: 12, color: 'var(--text-subdued)' }}>
              Inicio (ms)
              <input
                type="number" min={0} max={clip.end_time_ms - 1}
                value={clip.start_time_ms}
                onChange={e => handleFieldUpdate('start_time_ms', Number(e.target.value))}
                style={{ background: 'var(--bg-card-hover)', border: '1px solid var(--border)', borderRadius: 4, padding: '6px 8px', color: 'var(--text-base)', fontSize: 13 }}
              />
            </label>
            <label style={{ display: 'flex', flexDirection: 'column', gap: 4, fontSize: 12, color: 'var(--text-subdued)' }}>
              Fin (ms)
              <input
                type="number" min={clip.start_time_ms + 1}
