...
function parseLyrics(text: string, audioDuration: number): LyricsLine[] {
  ...
  const introTime = hasIntro
    ? audioDuration * 0.30
    : Math.min(8, audioDuration * 0.08)

  // Dynamic outro time
  const outroTime = hasOutro
    ? audioDuration * 0.08
    : audioDuration * 0.03
...
function buildTimedLines(
  storedLyrics: string,
  timestamps: LyricsSegment[]
): TimedLine[] {
  const rawLines = storedLyrics.split('\n').map(l => l.trim()).filter(l => l.length > 0)
  const sectionRegex = /^\[.+\]$/
  const cueRegex = /^\(.+\)$/

  // Count total sung (non-section, non-cue) lines for proportional mapping
  const totalLyricLines = rawLines.filter(
    t => !sectionRegex.test(t) && !cueRegex.test(t)
  ).length
  ...
  return rawLines.map(text => {
    if (sectionRegex.test(text)) {
      ...
      const nextSegIdx = totalLyricLines > 0
    ...
    if (cueRegex.test(text)) {
      const prevSegIdx = Math.max(0, Math.round((lyricIdx - 1) * (timestamps.length - 1) / Math.max(totalLyricLines - 1, 1)))
...
export default function LyricsView({ song, audioRef, onClose }: Props) {
  ...
  const timedLines = useMemo<TimedLine[] | null>(() => {
    if (!fullLyrics || !songTimestamps?.length) return null
    ...
    <div className={styles.overlay}>
      ...
      <div className={styles.lyricsWrapper}>
        ...
        <div className={styles.lyricsContainer}>
          {loading && (
            <div className={styles.message}>Cargando letra...</div>
          )}
          ...
          {!loading && !isInstrumental && visualLines.map((line, i) => {
            ...
            if (line.type === 'cue') {
              return <div key={i} className={styles.cueLine}>{line.text}</div>
            }
