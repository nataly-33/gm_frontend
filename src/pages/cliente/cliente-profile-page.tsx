...
function usePaymentMethods() {
  ...
  const add = async (pmId: string) => {
    setLoading(true)
    try {
      const pm = await clientService.savePaymentMethod(pmId)
        ...
        {pm.loading ? (
          ...
          <div className="flex flex-col gap-2 mt-3">
            {pm.methods.map((m) => (
              <div
                key={m.id}
                className="flex items-center justify-between rounded-lg px-4 py-3 bg-card-hover"
              >
