...
import {
  ...
  toggleLike,
  recordPlay,
} from '../../api/modules/community.api'
...
function SongCoverImage({ song, size = 240 }: { song: CommunitySong; size?: number }) {
  ...
  return (
    <img src={url} alt={song.title} style={{ width: '100%', height: size, objectFit: 'cover' }} />
  )
}
...
function SongCard({
  ...
  return (
    <article className="bg-[#121212] border border-[#1F1F1F] rounded-3xl overflow-hidden hover:border-[#2d2d2d] transition-all duration-200">
      {/* Cover */}
      ...
      <div className="p-5">
        ...
        <div className="mb-4">
          ...
          {song.tags.length > 0 && (
            <div className="flex flex-wrap gap-1 mt-2">
              {song.tags.map((t) => (
                <span
                  key={t.id}
                  className="text-xs bg-[#A855F7]/10 text-[#C084FC] px-3 py-1 rounded-full"
                >
                  {t.name}
                </span>
              ))}
          ...
          {song.description && (
            <p className="text-sm text-[#C7C7C7] mt-3 leading-relaxed line-clamp-2">
              {song.description}
            </p>
          )}
        </div>
        ...
        <div className="flex items-center justify-between border-t border-[#1F1F1F] pt-4">
          <div className="flex items-center gap-5 text-sm text-[#9CA3AF]">
            ...
            <div className="flex items-center gap-1.5">▶ {song.play_count.toLocaleString()}</div>
          </div>
        </div>
      </div>
    </article>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
        {[
          ...
          { label: 'Usuarios activos', value: stats?.active_users?.toLocaleString() ?? '…' },
          {
            ...
            value: stats
              ? stats.plays_today >= 1000
                ? `${(stats.plays_today / 1000).toFixed(1)}K`
                : stats.plays_today.toString()
              : '…',
          },
        ].map(({ label, value }) => (
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-24 text-center">
          ...
          <p className="text-[#9CA3AF] text-sm">
            Prueba con otro término o filtro, o sé el primero en publicar.
          </p>
        </div>
      ) : (
        <>
          ...
          {loadingPlay ? (
            ...
            <>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 2, minWidth: 160 }}>
                ...
                {activeSong.tags[0] && (
                  <span style={{ fontSize: 12, color: 'var(--color-primary-hover)' }}>
                    {activeSong.tags[0].name}
                  </span>
                )}
              </div>
              <audio
