  ...
  return (
    <div className={styles.card} onClick={onClick}>
      ...
      <p className={styles.cardMeta}>
        {playlist.song_count} canción{playlist.song_count !== 1 ? 'es' : ''}
      </p>
      <span
        className={`${styles.typeBadge} ${isAuto ? styles.typeBadgeAuto : styles.typeBadgeManual}`}
      >
        {isAuto ? 'Auto' : 'Manual'}
      </span>
    </div>
...
export default function PlaylistsPage() {
  ...
  async function handleFinishCreate() {
    ...
    try {
      ...
      if (ids.length > 0) {
        setManual((prev) =>
          prev.map((p) => (p.id === pendingPlaylist.id ? { ...p, song_count: ids.length } : p))
        )
      }
      closeCreateModal()
  ...
  return (
    <div className={styles.page}>
      <div className={styles.header}>
        <div>
          ...
          <p className={styles.pageSubtitle}>
            {tab === 'manual'
              ? 'Tus playlists creadas a mano'
              : 'Playlists generadas automáticamente por tus géneros y estados de ánimo'}
          </p>
        </div>
        {tab === 'manual' && (
          <button
            ...
            {createStep === 'title' ? (
              ...
              <>
                ...
                <p style={{ margin: 0, fontSize: 13, color: 'var(--text-subdued)' }}>
                  Selecciona las canciones para "{pendingPlaylist?.title}". Puedes agregar más desde
                  el detalle de la playlist.
                </p>
                <div
                  style={{
                    flex: 1,
                          ...
                          <div style={{ flex: 1, minWidth: 0 }}>
                            <p
                              style={{
                                ...
                                whiteSpace: 'nowrap',
                                color: selected ? 'var(--color-primary)' : 'var(--text-base)',
                              }}
                            >
                  ...
                  >
                    {addingSongs
                      ...
                      : selectedSongIds.size > 0
                        ? `Agregar ${selectedSongIds.size} y ver playlist`
                        : 'Ver playlist vacía'}
                  </button>
                </div>
              </>
      ...
      {loading ? (
        ...
        <div className={styles.grid}>
          {list.map((p) => (
            <PlaylistCard key={p.id} playlist={p} onClick={() => navigate(`/playlists/${p.id}`)} />
          ))}
        </div>
      ) : (
