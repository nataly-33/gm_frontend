...
const EMPTY_PLAN: Omit<CreditPlan, 'id'> = {
  slug: '', name: '', credits_per_month: 0, price_usd: '0.00',
  stripe_price_id: '', features: [], is_active: true,
}

const EMPTY_STRIPE: { name: string; price_usd: number; interval: 'month' | 'year' } = {
  name: '', price_usd: 0, interval: 'month',
}
...
export default function PlanesPage() {
  ...
  function handleEdit(plan: CreditPlan) {
    ...
    setForm({
      ...
      credits_per_month: plan.credits_per_month, price_usd: plan.price_usd,
  ...
  return (
    <div className="p-6 text-white">
      ...
      {tab === 'plans' && (
        <>
          <div className="bg-card-hover rounded-xl p-5 mb-8 flex flex-col gap-3 max-w-lg">
            ...
            ] as { label: string; key: keyof typeof form; type: string; ph: string }[]).map(({ label, key, type, ph }) => (
            ...
            <div className="flex gap-2 mt-1">
              ...
              {editId !== null && (
                <button onClick={() => { setEditId(null); setForm(EMPTY_PLAN) }}
          ...
          <div className="rounded-xl overflow-hidden border border-card-hover">
            <table className="w-full text-sm">
              ...
              <tbody>
                ...
                {plans.length === 0 && (
                  <tr><td colSpan={5} className="px-4 py-8 text-center text-muted text-sm">No hay planes registrados</td></tr>
                )}
              </tbody>
            </table>
          </div>
      ...
      {tab === 'stripe' && (
        <>
          <div className="bg-card-hover rounded-xl p-5 mb-8 flex flex-col gap-3 max-w-lg">
            <h3 className="text-sm font-bold text-muted uppercase tracking-wider mb-1">Crear Stripe Price</h3>
            <div className="flex flex-col gap-1">
              <label className="text-xs font-bold">Nombre del producto</label>
              <input value={stripeForm.name}
            ...
            <div className="flex flex-col gap-1">
              ...
              <input type="number" value={stripeForm.price_usd}
                onChange={e => setStripeForm(f => ({ ...f, price_usd: Number(e.target.value) }))}
                className="bg-surface rounded-md px-3 py-2 text-sm outline-none border border-transparent focus:border-primary text-white" />
            </div>
            <div className="flex flex-col gap-1">
              <label className="text-xs font-bold">Intervalo</label>
              <select value={stripeForm.interval}
                onChange={e => setStripeForm(f => ({ ...f, interval: e.target.value as 'month' | 'year' }))}
                className="bg-surface rounded-md px-3 py-2 text-sm outline-none border border-transparent focus:border-primary text-white">
                <option value="month">Mensual</option>
                <option value="year">Anual</option>
              </select>
            </div>
            <button onClick={handleCreateStripePrice} disabled={stripeLoading || !stripeForm.name}
          ...
          <div className="rounded-xl overflow-hidden border border-card-hover">
            <table className="w-full text-sm">
              ...
              <tbody>
                {prices.map((sp, i) => (
                  <tr key={sp.price_id} className={i % 2 === 0 ? 'bg-surface' : 'bg-night'}>
                    ...
                    <td className="px-4 py-3 text-right">
                      <button onClick={() => handleDeactivateStripePrice(sp.price_id)}
                        className="text-xs px-3 py-1 rounded-md bg-red-500/10 hover:bg-red-500/20 text-red-400">
                        Desactivar
                      </button>
                    </td>
                ...
                {prices.length === 0 && (
                  <tr><td colSpan={5} className="px-4 py-8 text-center text-muted text-sm">No hay prices en Stripe</td></tr>
                )}
              </tbody>
            </table>
          </div>
