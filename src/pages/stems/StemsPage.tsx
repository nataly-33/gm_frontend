...
import AudioUploader from '../../components/stems/AudioUploader'
import StemProgress from '../../components/stems/StemProgress'
import StemResultCard from '../../components/stems/StemResultCard'
...
export default function StemsPage() {
  ...
  async function handleSubmit(file: File) {
    ...
    setUploadError(null)
    try {
      const { job_id } = await uploadAndSeparate(file)
  ...
  return (
    <div className="p-6 flex flex-col gap-6 h-full">
      <div>
        ...
        <p className="text-muted text-sm mt-1">Separa vocals y karaoke de cualquier archivo de audio (2 créditos).</p>
      </div>
      ...
      <div className="grid gap-6" style={{ gridTemplateColumns: '1fr 1fr', flex: 1 }}>
        ...
        <div className="flex flex-col gap-4">
          ...
          {uploadError && (
            <p className="text-sm px-1" style={{ color: '#f87171' }}>{uploadError}</p>
          )}
