...
import { getMixProject, updateMixProject, startMixExport } from '../../api/modules/mix.api'
...
export default function MixEditorPage() {
  ...
  async function saveTitle() {
    if (!mix || !titleDraft.trim() || titleDraft === mix.title) {
  ...
  function handleClipAdded(clip: MixClip) {
    ...
    setMix((prev) =>
      prev ? { ...prev, clips: [...prev.clips, clip], clip_count: prev.clip_count + 1 } : prev
    )
    ...
    setMix((prev) =>
      prev
        ? {
            ...
            clip_count: Math.max(0, prev.clip_count - 1),
          }
        : prev
    )
  ...
  if (loading) {
    return (
      <div style={{ padding: '28px 32px', color: 'var(--text-subdued)', fontSize: 14 }}>
        Cargando editor...
      </div>
    )
  }
  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexShrink: 0 }}>
        ...
        {mix.bpm && (
          <span style={{ fontSize: 13, color: 'var(--text-subdued)' }}>{mix.bpm} BPM</span>
        )}
        ...
        >
          {selectedClipId ? (
                    ...
                    {[
                      ...
                      >
                        ...
                        <input
                          ...
                          onChange={async (e) => {
                            const updated = await (
                              await import('../../api/modules/mix.api')
                            ).updateClip(mix.id, clip.id, { [field]: Number(e.target.value) })
                  ...
                  {[
                    ...
                    >
                      ...
                      <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                        ...
                        />
                        <span style={{ fontSize: 11, minWidth: 44, color: 'var(--text-subdued)' }}>
                          {value}ms
                        </span>
                      </div>
                    </label>
                    ...
                    <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                      <input
                        ...
                        onChange={async (e) => {
                          const updated = await (
                            await import('../../api/modules/mix.api')
                          ).updateClip(mix.id, clip.id, { volume: Number(e.target.value) })
                          handleClipUpdated(updated)
                        }}
                        style={{ flex: 1, accentColor: 'var(--color-primary)' }}
                      />
                      <span style={{ fontSize: 11, minWidth: 36, color: 'var(--text-subdued)' }}>
            ...
            >
              ...
              <p style={{ fontSize: 14 }}>
                Selecciona un clip de la izquierda para editar sus parámetros
              </p>
            </div>
          )}
        </div>
